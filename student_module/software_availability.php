...
$labCards = [];
$result = $conn->query("SELECT lab, COUNT(*) AS total, SUM(status = 'installed') AS installed FROM software_applications GROUP BY lab ORDER BY lab ASC");
if ($result) {
    while ($row = $result->fetch_assoc()) {
        $labCards[] = $row;
    }
              ...
              <div class="card-body p-4">
                <div class="d-flex flex-wrap align-items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-muted mb-0">Check available software applications per laboratory before making a reservation.</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div class="card border shadow-sm h-100">
                  <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                      ...
                      <span class="badge text-bg-primary"><?= (int)$labCard['total'] ?> apps</span>
                    </div>
                    <p class="small text-muted mb-0"><?= (int)$labCard['installed'] ?> installed application(s)</p>
                  </div>
                </div>
              </div>
            <?php endforeach; ?>
          ...
          <div class="card-body p-4">
            <form method="GET" class="row g-3 align-items-end">
              <div class="col-lg-4">
                ...
                <input type="text" class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Search software, category, or version">
              </div>
              <div class="col-lg-3">
        ...
        <div class="card border shadow-sm">
          <div class="card-header bg-transparent border-bottom p-4">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
              <div>
                <h2 class="h5 mb-0"><i class="bi bi-list-check text-primary me-2"></i>Available Applications</h2>
                <small class="text-muted">Use this list to choose a lab that has the software you need.</small>
              </div>
          ...
          <div class="card-body p-0">
            <div class="table-responsive">
              <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                  <?php if (!$softwareList): ?>
                    <tr><td colspan="6" class="text-center text-muted py-5">No software records found.</td></tr>
                  <?php endif; ?>
      ...
      <div class="modal-content">
        ...
        <form action="../controllers/student/update_profile.php" method="POST">
          <div class="modal-body">
            ...
            <div class="row g-3">
              <div class="col-md-6"><label class="form-label">First Name</label><input type="text" name="firstname" value="<?= $firstname ?>" class="form-control"></div>
              <div class="col-md-6"><label class="form-label">Last Name</label><input type="text" name="lastname" value="<?= $lastname ?>" class="form-control"></div>
              <div class="col-md-6"><label class="form-label">Course</label><input type="text" name="course" value="<?= $course ?>" class="form-control"></div>
              <div class="col-md-6"><label class="form-label">Year Level</label><input type="text" name="yearlvl" value="<?= $yearlvl ?>" class="form-control"></div>
              <div class="col-12"><label class="form-label">Email</label><input type="email" name="email" value="<?= $email ?>" class="form-control"></div>
              <div class="col-12"><label class="form-label">Address</label><input type="text" name="addrs" value="<?= $addrs ?>" class="form-control"></div>
            </div>
          </div>
  ...
  <script>
    function applyDarkMode() {
      ...
      if (btn) {
        ...
        btn.innerHTML = enabled ? '<i class="bi bi-sun"></i><span>Light</span>' : '<i class="bi bi-moon-stars"></i><span>Dark</span>';
      }
    }
